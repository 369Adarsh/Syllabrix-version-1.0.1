    ...
    <div className="max-w-2xl mx-auto space-y-5">
      <FadeIn className="flex items-center justify-between">
        ...
        {notifs.some(n => !n.is_read) && (
          <motion.button whileTap={{ scale: 0.97 }} onClick={markAllRead} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-blue-600 hover:bg-blue-50 transition-colors"><CheckCheck size={14} /> Mark all read</motion.button>
        )}
      </FadeIn>
      ...
      : notifs.length === 0 ? (
        ...
        <StaggerChildren className="bg-white rounded-xl border border-gray-100 shadow-[0_1px_2px_rgba(0,0,0,0.1)] overflow-hidden divide-y divide-gray-50" stagger={0.04}>
          {notifs.map(n => (
            <StaggerItem key={n.id}>
              <div className={`flex items-start gap-3 p-4 transition-colors ${!n.is_read ? 'bg-blue-50/30' : 'hover:bg-gray-50'}`}>
