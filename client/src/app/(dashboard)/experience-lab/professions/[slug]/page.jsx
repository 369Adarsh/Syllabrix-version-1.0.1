...
export default function Page() {
  ...
  useEffect(() => { experienceAPI.getProfession(slug).then(r => setData(r.data?.data || [])).catch(() => {}).finally(() => setLoading(false)); }, []);

  return (
    <div className="max-w-3xl mx-auto space-y-5">
      <div className="flex items-center gap-3">
        <Link href="/experience-lab/professions" className="p-2 rounded-lg hover:bg-gray-100 transition-colors"><ArrowLeft size={16} className="text-gray-500" /></Link>
        ...
        <div><h1 className="font-bold text-lg text-gray-800">Profession Detail</h1><p className="text-[11px] text-gray-400">Deep dive into this profession.</p></div>
      </div>
      ...
      : !Array.isArray(data) || data.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-100 p-12 text-center shadow-sm">
          ...
          <p className="text-sm text-gray-400">Deep dive into this profession.</p>
        </div>
      ) : (
