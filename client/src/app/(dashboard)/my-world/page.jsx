...
import {
  Heart, MessageCircle, Plus, Sparkles, Star, ChevronRight, BookOpen, Loader2,
} from 'lucide-react';
...
function AICard({ card, className = '' }) {
  ...
  const bg = card.color || `bg-gradient-to-br ${bgMap[card.type] || 'from-blue-400 to-indigo-400'}`;

  return (
    <div
      className={`rounded-2xl overflow-hidden shadow-md flex-shrink-0 cursor-pointer select-none ${className}`}
      onClick={() => setOpen(o => !o)}
      style={{ background: card.color?.startsWith('from-') ? undefined : undefined }}
    >
      ...
      <div className="px-5 py-5">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-black text-white text-[18px] leading-tight">{challenge.title}</h3>
        ...
        <button
          ...
          disabled={done}
          className={`mt-4 w-full py-2.5 rounded-2xl font-black text-[14px] transition-all ${done ? 'bg-white/20 text-white/60' : 'bg-white text-orange-600 shadow-md hover:shadow-lg active:scale-[0.98]'}`}
        >
...
function FeedSkeleton() {
  return (
    <div className="space-y-4">
      {[1, 2, 3].map(i => (
        <div key={i} className="bg-white rounded-3xl p-4 space-y-3 animate-pulse">
...
function EmptyFeed({ onPost }) {
  return (
    <div className="bg-white rounded-3xl p-8 text-center border border-dashed border-amber-200">
...
export default function MyWorldPage() {
  ...
  // Redirect non-young-explorers away
  useEffect(() => {
  ...
  const loadFeed = useCallback(async () => {
    ...
    try {
      ...
      // Filter to young explorer content (posts with our activity hashtags or young users)
      setPosts(all);
    } catch {
  ...
  return (
    <div className="max-w-lg mx-auto space-y-5 pb-28 relative">
      {/* Header */}
      <div className="rounded-3xl overflow-hidden shadow-md"
        ...
        <div className="px-5 py-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[12px] font-black text-white/70 uppercase tracking-widest">{GREETING()}</p>
              <h1 className="text-[22px] font-black text-white leading-tight mt-0.5">{greetName}! 🌈</h1>
              {classLabel && (
                <span className="inline-block mt-1.5 px-3 py-0.5 bg-white/20 rounded-full text-[11px] font-bold text-white">
                  📚 {classLabel}
                </span>
              )}
            </div>
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={goPost}
              className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-white rounded-2xl text-orange-600 font-black text-[13px] shadow-md hover:shadow-lg active:scale-[0.97] transition-all">
              <Plus size={15} /> Share Today
            </button>
            <button
              onClick={() => router.push('/stories')}
              className="flex items-center gap-1.5 px-4 py-2.5 bg-white/20 rounded-2xl text-white font-bold text-[13px]">
      ...
      {/* Today's AI Picks — horizontal scroll */}
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-black text-gray-900 text-[15px] flex items-center gap-1.5"><Sparkles size={15} className="text-amber-500" /> Today's Picks</h2>
        </div>
        <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-hide">
          {cards.map((card, i) => (
            <AICard key={i} card={card} className="w-[220px]" />
          ))}
        </div>
      </div>
        ...
        <div className="flex gap-2 mb-4">
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex-1 py-2 rounded-xl font-black text-[13px] transition-all ${tab === t.id ? 'bg-amber-400 text-white shadow-md' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
              {t.label}
            </button>
          ))}
        </div>
            ...
            <div className="space-y-4">
              {filtered.map((post, i) => (
                <>
                  ...
                  {/* Inject AI card every 3 posts */}
                  {(i + 1) % 3 === 0 && i < filtered.length - 1 && (
                    <AICard key={`ai-${i}`} card={cards[i % cards.length]} />
                  )}
                </>
              ))}
            </div>
